<!--
  ARCHIVED — NOT IMPLEMENTED, AND NOT THE DESIGN SYSTEM OF THIS APP.

  This describes a light, boxy, 2010-era redesign that was specified and never
  built. It sat at `DESIGN.md` and claimed to be normative while every token in
  it disagreed with the shipped code: `scheme: light-only` against a hard-coded
  `APP_SCHEME = 'dark'`, `#f9f9f9` against `#121212`, Libre Franklin against
  Inter, square avatars against round, "no outer shadow, nowhere" against a
  component tree that uses `Elevation.card` throughout. Of the 18 colour token
  names it shared with `src/constants/theme.ts`, 17 held different values.

  It is kept because the thinking in it is real and a future direction may want
  it. Do not build from it, do not cite it as authority, and do not reconcile
  the code toward it without an explicit decision to change the product's whole
  visual identity. The live system is `DESIGN.md`.
-->
---
name: Social Playback
description: ARCHIVED, UNIMPLEMENTED — a 2010-era light redesign that was specified and never built. The live system is DESIGN.md.
scheme: light-only
target: react-native
colors:
  # Page and containers
...
This *would have replaced* a dark, cinema-led catalogue. That system's whole
logic — surface steps instead of borders, shadows for depth, poster-led
horizontal rails, a transparent floating header — is **inverted here**.

It never did replace it. The dark system is what shipped and what continues to
ship; it now lives at [DESIGN.md](DESIGN.md) and is the only normative spec in
this repo. Every sentence below is written in a present tense it never earned.
